...
export const CurrSeason = ({ loading, lastEpisodeToAir, nextEpisodeToAir, seasons, name }) => {
  const season = seasons?.find(item => lastEpisodeToAir.season_number === item.season_number);
          ...
          <section>
            ...
            <div className={styles.content}>
              ...
              {season?.overview ? (
                ...
                <p>
                  {season?.name} of {name} premiered on{' '}
                  {format(new Date(seasonAirDate), 'MMMM d, Y')}.
                </p>
              )}
              <p>
                <img
                ...
                {nextEpisodeToAir?.season_number || lastEpisodeToAir?.season_number}x
                {nextEpisodeToAir?.episode_number || lastEpisodeToAir?.episode_number},{' '}
                {format(new Date(nextEpisodeAirDate || lastEpisodeAirDate), 'MMMM d, Y')})
              </p>
            </div>
          </section>
...
CurrSeason.propTypes = {
  ...
  lastEpisodeToAir: PropTypes.shape({}).isRequired,
  nextEpisodeToAir: PropTypes.shape({}).isRequired,
