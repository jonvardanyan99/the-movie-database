...
export const Facts = ({ loading, homePage, status, networks, type, originalLanguage, budget, revenue }) => {
    const params = useParams();
    const {loading: externalIdsLoading, data: externalIds} = useQuery({ url: `/${params.mediaType}/${params.id}/external_ids`, params: '' });
    const {loading: keywordsLoading, data: keywords} = useQuery({ url: `/${params.mediaType}/${params.id}/keywords`, params: '' });
    ...
    return (
        <div className={styles.facts}>
            {(loading || externalIdsLoading || keywordsLoading) ? <Loader /> : (
                <>
                    {(externalIds?.facebook_id || externalIds?.twitter_id || externalIds?.instagram_id || homePage) && (
                        <div className={styles['link-container']}>
                            {externalIds.facebook_id &&
                            <div>
                                <a href={`https://www.facebook.com/${externalIds.facebook_id}`} target="_blank">
                                    <img src="https://www.themoviedb.org/assets/2/v4/glyphicons/social/facebook-71155d1cd369c47ce8456477833a92c324fa01e6d628cb6ece19cedea3c1c480.svg" alt="Facebook" />
                                </a>
                            ...
                            {homePage &&
                            <div className={styles.homepage} style={(!externalIds.facebook_id && !externalIds.twitter_id && !externalIds.instagram_id) ? {border: 'none', padding: 0} : null}>
                    ...
                    <>
                        ...
                        <div className={styles.networks}>
                            {networks.map(network => (
                                <img key={network.id} src={`https://www.themoviedb.org/t/p/h30${network.logo_path}`} alt={network.name} />
                            ))}
                        </div>
                    </>}
                    {params.mediaType === 'tv' &&
                    ...
                    <>
                        <h4>Budget</h4>
                        <p>{budget ? budget.toLocaleString('en-US', { style: 'currency', currency: 'USD', minimumFractionDigits: 2 }) : '-'}</p>
                        <h4>Revenue</h4>
                        <p>{revenue ? revenue.toLocaleString('en-US', { style: 'currency', currency: 'USD', minimumFractionDigits: 2 }) : '-'}</p>
                    </>}
