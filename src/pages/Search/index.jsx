...
export const Search = () => {
  ...
  const { loading: movieLoading, data: movie } = useQuery({
    url: '/search/movie',
    params: `&language=en-US&query=${queryParam}&page=${pageParam}&include_adult=false`,
  });
    ...
    <div className={styles.search}>
      <section>
        <div>
          ...
          <ul>
            <Link
              to={{ pathname: '/search/movie', search }}
              className={params.category === 'movie' ? styles.active : ''}
            >
              <li>
                <span>Movies</span>
                {movieLoading ? (
                  ...
                  <span className={styles['total-results']}>
                    {movie?.total_results.toLocaleString('en-US', {
            ...
            <Link
              to={{ pathname: '/search/tv', search }}
              className={params.category === 'tv' ? styles.active : ''}
            >
              <li>
                <span>TV Shows</span>
                {tvLoading ? (
                  ...
                  <span className={styles['total-results']}>
                    {tv?.total_results.toLocaleString('en-US', {
                      minimumIntegerDigits: 1,
                      useGrouping: true,
                    })}
                  </span>
            ...
            <Link
              to={{ pathname: '/search/person', search }}
              className={params.category === 'person' ? styles.active : ''}
            >
              <li>
                <span>People</span>
                {personLoading ? (
                  ...
                  <span className={styles['total-results']}>
                    {person?.total_results.toLocaleString('en-US', {
                      minimumIntegerDigits: 1,
                      useGrouping: true,
                    })}
                  </span>
      ...
      <div>
        <SearchDataDisplay loading={loading || false} data={data || {}} />
        {loading ? null : (
          <Pagination
